...
SELECT instructor_id, X.first_name, X.last_name
FROM instructor X LEFT OUTER JOIN student Y ON X.instructor_id = Y.advisor_id
WHERE Y.advisor_id IS NULL;

SELECT X.first_name, X.last_name, SUM(num_credits) AS total_cred
FROM instructor X INNER JOIN course Y ON X.instructor_id = Y.instructor_id
GROUP BY X.instructor_id, first_name, last_name;

SELECT course_code, course_name
FROM course
WHERE course_code LIKE '%3___';

SELECT course_code, first_name, last_name, SUM(num_credits) AS num_cred
FROM course X, student_schedule Y, instructor Z
WHERE X.course_id = Y.course_id AND X.instructor_id = Z.instructor_id AND student_id = 1
